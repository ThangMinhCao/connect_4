...
const IngamePage = ({ socket, userID }) => {
  ...
  useEffect(() => {
    ...
    const getMessages = async () => {
      try {
        await server_api.get(ENDPOINTS.getMessages, {
          headers: {
            token: localStorage.getItem('account_token')
          }, 
          params: {
    ...
    socket.on(`messages#${roomID}`, (data) => {
      setMessages(data.messages)
    });
    return () => {
      socket.removeAllListeners(`game#${roomID}`);
      socket.removeAllListeners(`messages#${roomID}`);
    }
  }, [roomID])
      ...
      : <div className={classes.page}>
          ...
          <div className={classes.gameBoard}>
            <GameBoard
              ...
              yourTurn={yourTurn}
            />
            <ChatBox userID={userID} roomID={gameInfo.id} messages={messages}  />
          </div>
          <EndGameDialog
            winner={winner}
            isGameEnd={isGameEnd}
